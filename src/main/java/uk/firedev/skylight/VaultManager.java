...
public class VaultManager {
    ...
    public boolean load() {
        ...
        if (!setupPermissions()) {
            Loggers.log(Level.WARNING, plugin.getLogger(), "Vault Permissions not found. Disabling Skylight.");
            plugin.getServer().getPluginManager().disablePlugin(plugin);
            return false;
        }
        if (!setupChat()) {
            Loggers.log(Level.WARNING, plugin.getLogger(), "Vault Chat not found. Disabling Skylight.");
            plugin.getServer().getPluginManager().disablePlugin(plugin);
            return false;
        }
        return true;
