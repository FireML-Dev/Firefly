package uk.firedev.firefly.gui;
...
public class ConfigGui {
    ...
    protected BaseGui createGui() {
        GuiType type = CommonUtils.getEnumValue(
            GuiType.class,
        ...
        PaperConfigReader reader = new PaperConfigReader(config);
        ComponentMessage<?, ?> title = ComponentMessage.componentMessage(reader, "title");
        if (title == null) {
    ...
    protected void addGuiItem(@NonNull BaseGui gui, @NonNull ConfigurationSection itemSection) {
        ...
        itemSection.getStringList("locations").forEach(location -> {
            ...
            String columnStr = CommonUtils.getOrDefault(splitLocation, 0, null);
            String rowStr = CommonUtils.getOrDefault(splitLocation, 1, null);
            int column = CommonUtils.getIntOrDefault(columnStr, -1);
            int row = CommonUtils.getIntOrDefault(rowStr, -1);
    ...
    protected void loadFiller(@NonNull BaseGui gui) {
        ...
        // Prepare Enum
        FillerType fillerType = CommonUtils.getEnumValue(
            FillerType.class,
        ...
        ItemType itemType = CommonUtils.getItemType(
        ...
        switch (fillerType) {
            case ALL -> {
                if (gui instanceof PaginatedGui) {
                    Firefly.getInstance().getLogging().warn("Paginated GUIs cannot use FillerType.ALL");
                    return;
                }
                filler.fill(item);
            }
            ...
            case SIDE -> {
                GuiFiller.Side side = CommonUtils.getEnumValue(
